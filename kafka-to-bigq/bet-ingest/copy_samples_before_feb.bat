...
psql -c "\copy match FROM 'csv_enrich\match_"%1".csv' delimiter ';' csv HEADER" -U postgres postgres
psql -c "\copy runner FROM 'csv\runner_"%1".csv' delimiter ';' csv HEADER" -U postgres postgres
...
rem curl --location --request POST "http://localhost:9000/scraper/recover" --data-raw "{""to"" : ""2021-12-27T23:00:00"", ""since"" : ""2021-12-27T00:00:00""}" -H "Content-Type: application/json"
...
python pipeline_before_feb.py --bootstrap_servers localhost:9092 --match_csv match_%1.csv --runner_csv runner_%1.csv --out_csv data.csv
